...
class FileUpload
{
    ...
    protected $error_messages = array(
        1 => '上傳的文件超出了 php.ini 中的 upload_max_filesize 設定',
        2 => '上傳的文件超過了HTML表單中指定的 MAX_FILE_SIZE 設定',
        3 => '上傳的文件僅部分成功上傳',
        4 => '沒有文件上傳',
        6 => '缺少暫存文件夾',
        7 => '無法將文件寫入磁盤',
        8 => 'PHP擴展程序停止了文件上傳',
        'post_max_size' => '上傳的文件超出了php.ini中的 post_max_size 設定',
        'max_file_size' => '文件太大',
        'min_file_size' => '文件太小',
        'accept_file_types' => '不允許的文件類型',
        'max_number_of_files' => '超出最大文件數',
        'invalid_file_type' => '副檔名與檔案類型不一致',
        'max_width' => '圖片超出最大寬度',
        'min_width' => '圖片需要最小寬度',
        'max_height' => '圖片超出最大高度',
        'min_height' => '圖片要求最小高度',
        'abort' => '文件上傳中止',
        'image_resize' => '無法調整圖像大小'
    );
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|doc|pdf)$/i',
            ...
            'accept_file_types' => '/\.(gif|jpe?g|png|doc|pdf)$/i',
    ...
    protected function get_full_url() {
        return $this->get_domain().substr($_SERVER['SCRIPT_NAME'],0, strrpos($_SERVER['SCRIPT_NAME'], '/'));
    }
    ...
    protected function set_additional_file_properties($file) {
        ...
        $file->data->deleteUrl = $this->options['script_url']
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->name);
        $file->data->saveDeleteUrl = str_replace($this->get_domain(),'',$file->data->deleteUrl);
        $file->data->deleteType = $this->options['delete_type'];
        if ($file->data->deleteType !== 'DELETE') {
            $file->data->deleteUrl .= '&_method=DELETE';
        }
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
            ...
            $file->saveUrl = str_replace($this->get_domain(),'',$file->url);
            foreach ($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    if (is_file($this->get_upload_path($file_name, $version))) {
                        ...
                        $file->{'save'.$version.'Url'} = str_replace($this->get_domain(),'',$file->{$version.'Url'});
                    }
                }
            }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        // 儲存原始資料
        $file->original = pathinfo($name,PATHINFO_FILENAME);
        $upload_name = md5(uniqid().time()).'.'.pathinfo($name, PATHINFO_EXTENSION);

        $file->name = $this->get_file_name($uploaded_file, $upload_name, $size, $type, $error, $index, $content_range);

<template>
  <li :class="dropdownClasses">
    ...
    <ul class="c-sidebar-nav-dropdown-items" style="background:#293547;" @click="itemClicked">
      <slot>
        <template v-for="(item, key) in items">
          <template v-if="item.childrens.length==0">   
             <CSidebarNavItem :name="item.name" :icon="item.icon" :key="key" :to="item.url"/>
          </template>
          <template v-else>
            <CSidebarNavDropdown2 :name="item.name" :icon="item.icon" :items="item.childrens"></CSidebarNavDropdown2>
          </template>
...
import CSidebarNavDropdown2 from './CSidebarNavDropdown2.vue'
export default {
  name:'CSidebarNavDropdown',
  components: {
    CIcon,
    CSidebarNavItem,
    CSidebarNavDropdown2
  },
  props: {
    name: String,
    route: {
